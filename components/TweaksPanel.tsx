...
interface TweakOptionObj {
  value: string;
  label: string;
}
type TweakOption = string | TweakOptionObj;
...
function useTweaks(defaults: TweakValues): [TweakValues, SetTweak] {
  ...
  const setTweak: SetTweak = React.useCallback((key: string, val: any) => {
    const edits = { [key]: val };
...
function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  const opts: TweakOptionObj[] = options.map((o) => (typeof o === 'object' ? o : { value: o, label: o }));
